...
class Mapper(object):
    ...
    def scan_to_pcl(self, r, angle):
        valX = r * math.cos(angle)
        valY = r * math.sin(angle)

    def scan_callback(self, scan):
        """ Update the map on every scan callback. """
        ranges = scan.ranges
        increment = scan.angle_increment
        angle = scan.angle_min - increment
        angMin = scan.angle_min
        angMax = scan.angle_max
        points = []

        for r in ranges:
            angle += increment
            points.append(self.scan_to_pcl(r, angle))
